...
export class UsersService {
   ...
   async createUser(userData: CreateUserDto, res: Response) {
    ...
    async updateProfile(userData: UpdateUserDto){
        ...
        //const dataUserss = await this.userRepository.findOne({where:{id: userData.idUsers}});
        console.log(dataUser);
        //console.log(dataUsers);
        //dataUserss?.fullname = userData.fullname ?? dataUserss?.fullname;
    ...
    async UserInfo(id: string, res: Response){
        try {
            const userInfo = await this.userRepository.findOne({
            where: { id: id },
            //select: ["id","pseudo","fullname","email","telNumber","password","countPrompt","lastPromptDate","isActive","created_at","emailVerify"],
